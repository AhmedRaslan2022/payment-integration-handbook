...
> ⚠️ Always review your gateway’s official documentation carefully, as request formats, signature headers, and field names can vary between providers.
...
Refer to your gateway’s documentation — typical parameters include:
...
1. User completes payment → Gateway redirects to `success_url`, `failure_url`, or `cancel_url`.  
2. Frontend displays an appropriate confirmation, error, or cancellation screen.  
3. Frontend calls your backend to **recheck payment status** from your database or via the **gateway’s Payment Details API**.  
4. Backend confirms the latest order status to the frontend for accurate display.

> ⚠️ **Important:** Never rely solely on redirection for payment confirmation.  
...
| Stage | Description |
|--------|-------------|
| **Authorization** | Amount is reserved on the customer’s card (no funds transferred yet). |
| **Capture** | Merchant explicitly captures (all or part of) the authorized amount after successful checkout. |

### ⚙️ Implementation Notes

- Always capture payments using the gateway’s **Capture API**.  
...
2. **Use idempotency**  
   - If a webhook or capture call repeats, ensure it’s processed only once using `event_id` or `transaction_id` checks.
